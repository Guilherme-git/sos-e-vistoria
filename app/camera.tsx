...
export default function CameraScreen() {
  ...
  const { photoType, inspectionType, photoIndex } = useLocalSearchParams<{
    photoType: string;
    inspectionType: string;
    photoIndex: string;
  }>();
  const { activeCall, updateActiveCall } = useCalls();
  ...
  const savePhoto = async (uri: string) => {
    ...
    if (inspectionType === 'checkin') {
      const photos = [...activeCall.checkinPhotos];
      photos[index] = uri;
      await updateActiveCall({ checkinPhotos: photos });
    } else if (inspectionType === 'checkout') {
      const photos = [...activeCall.checkoutPhotos];
      photos[index] = uri;
      await updateActiveCall({ checkoutPhotos: photos });
    } else if (inspectionType === 'belongings') {
      const photos = [...activeCall.belongingsPhotos];
      photos[index] = uri;
      await updateActiveCall({ belongingsPhotos: photos });
    }
  ...
  return (
    <View style={[styles.screen, { paddingTop: topInset }]}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>{photoType || 'Foto'}</Text>
        <View style={{ width: 40 }} />
      </View>
...
const styles = StyleSheet.create({
  ...
  successOverlay: {
    ...
    justifyContent: 'center',
    alignItems: 'center',
    backgroundColor: 'rgba(13, 27, 22, 0.5)',
    gap: 12,
  },
  successBadge: {
    width: 64,
